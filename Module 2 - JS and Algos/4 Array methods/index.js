...
const ans1 = users.map(user => { return { email: user.email, companyName: user.company.name } })
console.log('--- Answer 1: ', ans1);


// Ex2
const ans2 = users.filter(user => user.address.zipcode[0] === "5")
console.log('--- Answer 2: ', ans2);


// Ex3
const ans3 = users.filter(user => user.address.zipcode[0] === "5").map(item => item.id)
console.log('--- Answer 3: ', ans3);


// Ex4
const ans4 = users.filter(user => user.name[0].toLocaleLowerCase() === "c").map(user => user.name);
console.log('--- Answer 4: ', ans4);


// Ex5
const ans5 = users.every(user => user.address.city === 'South Christy');
console.log('--- Answer 5: ', ans5);


// Ex6
const ans6 = users.filter(user => user.address.suite === 'Apt. 950')[0].company.name;
console.log('--- Answer 6: ', ans6);


// Ex7
function formatUser(user) {
    return `${user.name} lives in ${user.address.city} and own the compeny ${user.company.name}`;
}
...
const ans8 = inventory.reduce((prev, curr) => prev + (curr.price * curr.quantity), 0);
...
const ans9 = studentScores.reduce((prev, curr) => {
    let grade;
    if (curr > 90) grade = 'A';
    else if (curr > 80) grade = 'B';
    else if (curr > 70) grade = 'C';
    else grade = 'F';
...
let taxRates = {
    clothing: 1.08,    // 8% tax
    electronics: 1.10, // 10% tax  
    food: 1.05        // 5% tax
}

const ans10 = cartItems.reduce((prev, curr) => {
    return prev + curr.price * curr.quantity * taxRates[curr.category]
}, 0)
